...
select c.nom_categ  from categorie c  
where  c.nb_points= 3;
...
select t.num_trophee  from trophee t 
...
where h.nom like 'A%' and h.nom like '%r%';

--6. Numéros des habitants ayant bu les potions numéros 1, 3 ou 4. (8 lignes)
select DISTINCT  a.num_hab from absorber a  
...
where p.lib_potion = 'Potion magique n°1' and a.date_a 
...
where v.num_village = r.num_village ;
...
select h.nom, count(t.num_preneur) from habitant h 
join trophee t on h.num_hab = t.num_preneur 
group by h.nom having count(t.num_preneur)  
> 
(select count(t.num_preneur) from trophee t join habitant h  on t.num_preneur = h.num_hab where h.nom = 'Obélix' group by h.nom); 
